...
export default function Signup({ onClose }){
  ...
  const submit = async () => {
    ...
    setLoading(true)
    try{
      const res = await fetch('/api/signup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, source: 'web-portal' }) })
      const data = await res.json()
      setLoading(false)
      alert('Signup received: ' + (data.message || JSON.stringify(data)))
      onClose && onClose()
    }catch(e){
      setLoading(false)
